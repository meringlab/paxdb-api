        ...
        if (Object.prototype.hasOwnProperty.call(paxdbUniprotIdsMap, r.protein_external_id)) {
          ...
          if (!Object.prototype.hasOwnProperty.call(uniprotPaxdbIdsMap, ac)) {
            ...
            if (!Array.isArray(prev)) {
              console.log(`multiple paxdb ids for ${ac}, must to use an array`);
              uniprotPaxdbIdsMap[ac] = [prev];
            }
            uniprotPaxdbIdsMap[ac].push(r.protein_id);
          }
        }
...
/**
 * depends on proteins data!
 */
function build_proteins_index(){
  "use strict";

  let speciesForProtein = {};
  let externalToInternalMap = {};
  let uniprotIdsMap = {};
  console.log('reading proteins and uniprot mapping');
  ...
  uniprotLinkins.split("\n").forEach((line) => {
    if (line) {
      ...
      if (currentSpecies != speciesId) {
        ...
        proteins = require(`./lib/proteins/${speciesId}.js`);
        externalToInternalMap = {};
        for (let proteinId in proteins) {
          if (proteins.hasOwnProperty(proteinId)) {
            let p = proteins[proteinId];
            speciesForProtein[proteinId] = speciesId;
            externalToInternalMap[p.externalId] = p.id;
          }
        }
      }
  ...
  if (exportedSpecies.size != speciesIds.length) {
    let missing = [...speciesIds].filter(s => !exportedSpecies.has(s));
    throw Error(`failed to find all species, missing: ${missing}`);
  }
  ...
  writeStream.write('module.exports.uniprotIdsMap = uniprotIdsMap;');
  writeStream.end();
  console.log('proteins_index.js written');
